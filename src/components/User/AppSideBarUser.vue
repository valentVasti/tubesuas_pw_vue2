	...
	<div>
		<v-navigation-drawer app v-model="drawer" width="16%" color="#219ebc" hide-overlay>
			...
			<v-list nav>
				<v-list-item class="my-5" v-for="menu in menus" :key="menu.title" link tag="router-link" :to="menu.to">
					...
					<v-list-item-content>
						<v-list-item-title
							style="font-family: Poppins; color: #ffff; font-size: 110.5%; font-weight: 600;">{{
									menu.title
							}}</v-list-item-title>
					</v-list-item-content>
				</v-list-item>
		...
		<div class="fluid">
			<v-app-bar app fixed height="80%" color="#ffff">
				...
				<v-spacer></v-spacer>
				<v-btn color="#ef233c"
					style="font-family: Poppins; font-size: 20px; text-transform: capitalize; font-weight: 700; color:#ffff;"
	...
	data() {
		return {
			...
			menus: [
				...
				{ title: 'Transaksi', icon: 'mdi-cash', to: '/transaksiPage' },
			],
		}
	},
	setup() {
